...
(function( root, factory ) {
	...
	function setupDOM() {
		...
		dom.who = document.querySelector( '.who' );
	...
	function updateProgress() {
		...
		if( config.progress && dom.progressbar ) {
			if((getProgress() * dom.wrapper.offsetWidth) > 50) {
				if (!dom.who.classList.contains('show')) {
					dom.who.classList.add('show');
				}
			}

			if((getProgress() * dom.wrapper.offsetWidth) > dom.wrapper.offsetWidth - 50) {
				if (dom.who.classList.contains('show')) {
					dom.who.classList.remove('show');
				}
			}

			dom.who.style.left = getProgress() * dom.wrapper.offsetWidth + 'px';
	...
	function updateSlideNumber() {
		...
		if( config.slideNumber && dom.slideNumber) {
			// console.log(config.speakerToggles.indexOf(config.slideNumber));
			if(config.dries.indexOf(getSlidePastCount() + 1) !== -1 ){
				dom.who.classList.add('flipped');
			} else {
				dom.who.classList.remove('flipped');
			}
